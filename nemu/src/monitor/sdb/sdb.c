...
static int cmd_test(){
  FILE *fp = fopen("/home/ubuntu/caichi/ysyx-workbench/nemu/tools/gen-expr/build/input","r");
  if (fp == NULL)
  {
    printf("Test file input not found!\n");
    return 0;
  }
  
  bool make_token_var = true;
  bool *make_token = &make_token_var;
  char str[65546];
  word_t result, result_eval = 0;
  char *expression, *result_str = NULL;
  ...
  while (fgets(str, sizeof(str), fp) != NULL)
  {
    ...
    result_str = str;
    expression = str + mark + 1;
    result_eval = expr(expression, make_token);
    
    if (*make_token == false)
    {
      printf("Unrecognized token in expression, making token failed!\n");
      continue;
    }

    result = strtoul(result_str, NULL, 10);
    
    printf("Expression:\"%s\"\n",  expression);
    printf("Result:%u\n", result);
    printf("Evaluated result:%u\n", result_eval);
    if(result == result_eval) printf("Results match!\n");
    else printf("Results mismatch!\n");
    printf("\n\n");
  }
...
static struct {
  ...
  { "test", "Test expression evaluation function(need test file generated by gen-expr tool under tools directory before running this command!)", cmd_test },
